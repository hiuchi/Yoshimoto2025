...
p1 <- ggplot(data_box, aes(x = HLA_B_LOH, y = HLA_expression, fill = HLA_B_LOH)) +
  geom_boxplot() +
  geom_jitter(width = 0.2, size = 1, alpha = 0.6) +
  scale_fill_manual(values = colors,
                    name = "HLA-B LOH status") +
...
# met_aveのボックスプロット
p2 <- ggplot(data_box, aes(x = HLA_B_LOH, y = met_ave, fill = HLA_B_LOH)) +
  geom_boxplot() +
  geom_jitter(width = 0.2, size = 1, alpha = 0.6) +
  scale_fill_manual(values = colors,
                    name = "HLA-B LOH status") +
...
# CD8_numberのボックスプロット
p3 <- ggplot(data_box, aes(x = HLA_B_LOH, y = `CD8_number`, fill = HLA_B_LOH)) +
  geom_boxplot() +
  geom_jitter(width = 0.2, size = 1, alpha = 0.6) +
  scale_fill_manual(values = colors,
                    name = "HLA-B LOH status") +
...
create_pie_chart <- function(data, column, title) {
  ...
  p <- ggplot(df_proportions, aes(x = "", y = proportion, fill = !!sym(column))) +
    ...
    labs(title = title, fill = "HLA-B LOH status") +  # 凡例のタイトルを日本語に
...
# HLA_B_LOH列の円グラフを作成
create_pie_chart(data_km, "HLA_B_LOH", "HLA-B LOH status")
```
...
fit <- survfit(surv_obj ~ HLA_B_LOH, data = data_km)
...
km_plot <- ggsurvplot(fit,
                      ...
                      title = "Kaplan-Meier Curve by HLA-B LOH Status",  # タイトル
...
plot_hpvp <- ggplot(data_hpvp, aes(x = factor(HPV_positive), fill = factor(HLA_B_LOH))) +
  geom_bar(position = "dodge") +
  scale_fill_manual(values = colors, name = "HLA_B_LOH") +  # カスタムカラーを適用
  labs(title = "Distribution of HLA_B_LOH by HPV_positive", 
       x = "HPV_positive", 
       y = "Count") + 
  theme_minimal()
...
# 各HPV_positiveグループ内でHLA_B_LOHの割合を計算
data_hpvp_normalized <- data_hpvp %>%
  group_by(HPV_positive, HLA_B_LOH) %>%
  summarise(count = n()) %>%
  mutate(proportion = count / sum(count) * 100)

# HLA_B_LOHのレベルの順序を逆に指定（"ROH"を先に）
data_hpvp_normalized$HLA_B_LOH <- factor(data_hpvp_normalized$HLA_B_LOH, levels = c("ROH", "LOH"))

# 積み上げグラフを作成
plot_stack_normalized <- ggplot(data_hpvp_normalized, aes(x = factor(HPV_positive), y = proportion, fill = HLA_B_LOH)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_manual(values = rev(custom_colors), name = "HLA_B_LOH") +  # カラーも逆順に
  labs(title = "Proportion of HLA_B_LOH by HPV_positive (Stacked 100%)", 
       x = "HPV_positive", 
       y = "Proportion (%)") + 
  theme_minimal()
...
```{r test}
# 分割表を作成
cont_table <- table(data_hpvp$HLA_B_LOH, data_hpvp$HPV_positive)
print("Contingency Table (HLA_B_LOH vs HPV_positive):")
print(cont_table)
...
pdf("FigS3HLA-B,box plot.pdf", width = 6, height = 4)
...
```{r}
# 現在の作業ディレクトリ（RMDファイルと同じ場所）にPDFを保存
pdf("FigS3HLA-B_2,box plot.pdf", width = 8, height = 6)
