...
pHistology_met <- create_pie_chart_met(data, "Histology", 
                                      "Distribution of HLA methylation\nby histological type",
                                      is_histology = TRUE)
pHPV_met <- create_pie_chart_met(data, "HPV_positive", 
                                "Distribution of HLA methylation\nby HPV status",
                                is_histology = FALSE)
```
...
data$Histology <- factor(data$Histology, levels = c("SCC", "AC", "GAS", "Small"))
#組織型別のHLAメチル化の箱ひげ図
pHist_metx <- ggplot(data, aes(x = Histology, y = met_ave, fill = Histology)) +
  geom_boxplot() +
  geom_jitter(width = 0.2, size = 1, alpha = 0.6) +
  scale_fill_manual(values = all_colors[c("SCC", "AC", "GAS", "Small")]) +
  stat_compare_means(method = "wilcox.test",
